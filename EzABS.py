			...
			if args["message"] == "Templates":
				...
				if not body["empty"]:
					...
					if response == "successful":
						...
						success = False
						code = 906
					preparedPayload = '{ "command": "SENSOR_STATUS", "mac_address": "'+ str(hex(uuid.getnode()))+'", "success": "' +str(success)+'", "message": "'+ response +'", "code": "'+ str(code) +'", "scannerId": "'+ str(content["scannerId"]) +'", "id": "' + str(content["uid"]) + '", "userId": "' + str(content["emp"]["uid"]) + '" }'
